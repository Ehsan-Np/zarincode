...
### `GET /courses`
عمومی است و با سقف ۶۰ درخواست در دقیقه محدود می‌شود. فهرست دوره‌های منتشرشده، قیمت، تعداد جلسه و تصویر را برمی‌گرداند. پارامتر `page` صفحه‌بندی می‌کند.

### `GET /progress`
نیازمند ورود است. پیشرفت دوره‌های کاربر جاری و لینک کلاس درس را برمی‌گرداند.

### `POST /progress`
نیازمند ورود است. بدنه: `course_id`, `lesson_key`, `seconds`, `duration`, `complete`. تکمیل جلسه فقط وقتی ثبت می‌شود که درصد تماشا به آستانهٔ پنل رسیده باشد.

### `GET /wallet`
نیازمند ورود است. موجودی و ده تراکنش آخر را برمی‌گرداند.

### `GET /tickets`
نیازمند ورود است. فهرست تیکت‌های کاربر جاری را برمی‌گرداند.

### `POST /webhooks`
فقط مدیر. ثبت وب‌هوک خروجی HTTPS با `url`, `events` و `secret` حداقل ۱۶ نویسه. رویدادها با هدر `X-Zarincode-Sign` (HMAC-SHA256 روی بدنه) ارسال می‌شوند.
